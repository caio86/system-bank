...
export class ContaService {
  ...
  inserir(novoConta: Conta): Observable<Conta> {
  ...
  listar_paginado(page: number, pageSize: number): Observable<Conta[]> {
    return this.clienteHttp.get<Conta[]>(
      `${this.api}?page=${page}&pageSize=${pageSize}`
    )
  }

  deletar(idConta: number): Observable<object> {
    return this.clienteHttp.delete<object>(`${this.api}${idConta}`)
  }
  ...
  saque(saque: SaqueDeposito): Observable<SaqueDeposito> {
    return this.clienteHttp.post<SaqueDeposito>(
      `${this.api}${saque.conta}/saque/`,
      saque
    )
  }
  ...
  deposito(deposito: SaqueDeposito): Observable<SaqueDeposito> {
    return this.clienteHttp.post<SaqueDeposito>(
      `${this.api}${deposito.conta}/deposito/`,
      deposito
    )
  }

  transferencia(transferencia: Transferencia): Observable<Transferencia> {
    return this.clienteHttp.post<Transferencia>(
      `${this.api}${transferencia.conta_origem}/transferencia/`,
      transferencia
    )
  }
}
